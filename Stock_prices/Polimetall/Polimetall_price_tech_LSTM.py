...
based on the close price and technical indicators for previous periods. Multivariate LSTM model.
...
def plot_history(hist):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('multi_training.png')
    plt.show()
...
model = tf.keras.models.Sequential(
    [
        normalizer,
        tf.keras.layers.LSTM(16, recurrent_dropout=0.15, return_sequences=True),
        tf.keras.layers.LSTM(8, recurrent_dropout=0.15, return_sequences=False),
...
plt.legend()
plt.savefig('multi_forecast.png')
plt.show()
