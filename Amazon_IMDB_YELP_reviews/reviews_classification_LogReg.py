...
# с использованием алгоритма Logistic Regression.
...
lr.fit(X_train_num, y_train)  # Обучаем модель
y_pred_class = lr.predict(X_test_num)  # Делаем прогноз
...
y_pred_prob = lr.predict_proba(X_test_num)[:, 1]
...
y_pred_class = lr.predict(X_test_num)
...
# Делим все отзывы на учебные и тестовые:
X_train, X_test, y_train, y_test = train_test_split(data.review, data.sentiment, random_state=2)

# Преобразуем тексты из всех источников в числовую матрицу:
X_train_num = vect.fit_transform(X_train)
X_test_num = vect.transform(X_test)
